...
const Pin = ({ pin }) => {
  const { post, culture, image, _id, destination, title } = pin;
  // console.log(pin);
  // console.log(culture);
  // console.log(title);
  ...
  return (
    <div className={`m-2 py-3 ${deleted ? "hidden" : null}`}>
      ...
      >
        ...
        {image && (
          <div className="flex flex-col items-center">
            <img className="w-full rounded-xl " src={urlFor(image).width(250).url()} alt="user-post" />
          </div>
        )}
        {/* When post is hovered */}
        {postHovered && (
          <div className="absolute top-0 z-50 flex h-full w-full cursor-pointer flex-col justify-between p-1 pr-2 pt-2 pb-2" style={{ height: "100%" }}>
            <div className="flex items-center justify-between">
              ...
              {alreadySaved?.length !== 0 ? (
                <button type="button" className="rounded-3xl bg-orange-500 px-5 py-1 text-base font-bold text-white opacity-70 outline-none hover:opacity-100 hover:shadow-md">
                  Saved
                </button>
              ) : (
                <button
                  ...
                  type="button"
                  className="rounded-3xl bg-orange-500 px-5 py-1 text-base font-bold text-white opacity-70 outline-none hover:opacity-100 hover:shadow-md"
                >
            ...
            <div className=" flex w-full items-center justify-between gap-2">
              {destination?.slice(8).length > 0 ? (
                <a href={destination} target="_blank" className="flex items-center gap-2 rounded-full bg-white p-2 pl-4 pr-4 font-bold text-black opacity-70 hover:opacity-100 hover:shadow-md" rel="noreferrer">
      ...
      <div className="flex flex-col items-center justify-center">
        <span className="h1 text-center font-bold text-red-400">{title && title}</span>
        {/* Show the user who posted it */}
        <Link to={`/user/${post?._id}`} className="flex items-center gap-2 ">
          <img className="h-8 w-8 rounded-full object-cover" src={post?.image} alt="user" />
          <p className="text-sm font-thin capitalize">{post?.userName}</p>
          <hr />
        </Link>
      </div>
    </div>
