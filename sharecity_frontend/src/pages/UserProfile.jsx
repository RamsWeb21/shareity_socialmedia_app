...
const UserProfile = ({ image }) => {
  ...
  const [text, setText] = useState("Creaed");
      ...
      <div className="flex flex-col pb-5">
        <div className="relative mb-7 flex flex-col">
          <div className="flex flex-col items-center justify-center">
            <img className="2xl:h-50 h-40 w-full bg-white object-cover opacity-100 shadow-lg " src={logo} alt="user-pic" />
            <img className="-mt-10 h-20 w-20 rounded-full object-cover shadow-xl" src={user.image} alt="user-pic" />
          </div>
          ...
          <div className="z-1 absolute top-0 right-0 p-2">
            {userId === User.uid && (
              ...
              <button type="button" className=" cursor-pointer rounded-full bg-white p-2 shadow-md outline-none" onClick={() => FirebaseLogout()}>
        ...
        {pins?.length === 0 && <div className="text-1xl mt-2 flex w-full items-center justify-center font-bold">No Pins Found!</div>}
      </div>
      <Footers />
    </div>
  );
