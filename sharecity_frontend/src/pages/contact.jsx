...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "sservice_47nu0jf", // service id
        "template_cnaka3l", // template id
        form.current,
        "10A-93H_atnPz8f4S9Lpx"
      )
      .then(
        (result) => {
    ...
    <>
      <section className="bg-white ">
        <div className="mx-auto max-w-screen-md px-4 lg:py-16">
          ...
          <p className=" text-center font-light text-gray-500 lg:mb-16">Got a technical issue? Want to send feedback about a beta feature?</p>
          {/* Form */}
          <form ref={form} onSubmit={sendEmail} className="gap-5">
            {/* Name */}
            <div>
              <label className="mt-5 block text-sm font-medium text-gray-900 dark:text-gray-300">Name</label>
              <input
                type="text"
                name="name"
            ...
            {/* email */}
            <div>
              <label className="mt-5 block text-sm font-medium text-gray-900 dark:text-gray-300">Email</label>
              <input
                type="email"
                name="email"
            ...
            <div className="sm:col-span-2">
              <label className="mt-5 block text-sm font-medium text-gray-900 dark:text-gray-400">Your message</label>
              <textarea
                name="message"
            ...
            <div className="flex flex-row justify-center">
              <input type="submit" value="Send" className="m-5 cursor-pointer rounded bg-red-500 py-2 px-4 font-bold text-white hover:bg-red-700 " />
            </div>
          </form>
        </div>
      </section>
